...
func BackupScreenshots(version string) error {
	screenshotDir := filepath.Join(GetGameDir(), version, ScreenshotsDir)
...
func BackupUserDirectory(version string) error {
	userDir := filepath.Join(GetGameDir(), version, UserDir)
...
func BackupUserCharacters(version string) error {
	charDir := filepath.Join(GetGameDir(), version, CharactersDir)
...
func BackupControlMappings(version string) error {
	mappingsDir := filepath.Join(GetGameDir(), version, ControlMappingsDir)
...
func RestoreControlMappings(version string, filename string) error {
	mappingsFilePath := filepath.Join(GetGameDir(), version, ControlMappingsDir, filename)
